...
from .models import DodatkoweInfo, Film, Ocena, Aktor
from .forms import FilmForm, DodatkoweInfoForms, OcenaForm, AktorForm
...
def nowy_film(request):
    ...
    form_dodatkowe = DodatkoweInfoForms(request.POST or None)
    ...
    'form_dodatkwe': form_dodatkowe, 'nowy': True})
...
def edytuj_film(request, id):
    ...
    Dodaj_aktora = Aktor.objects.filter(filmy=film)
    ...
    form_aktorzy = AktorForm(request.POST or None)
    ...
    if request.method == 'POST':
        if 'imie' in request.POST:
            Dodaj_aktora = form_aktorzy.save(commit=False)
            Dodaj_aktora.film = film
            Dodaj_aktora.save()
    ...
    return render(request, 'film_form.html', {'form': form_film,
    'form_dodatkwe': form_dodatkowe, 'oceny': oceny, 'form_ocena': form_ocena,
     'form_aktorzy': form_aktorzy, 'aktorzy': aktorzy, 'Dodaj_aktora': Dodaj_aktora, 'nowy': False})
